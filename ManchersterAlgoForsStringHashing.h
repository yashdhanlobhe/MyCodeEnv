...
template<typename A, typename B> omStringeam& operator<<(omStringeam &os, const pair<A, B> &p) { return os << '(' << p.first << ", " << p.second << ')'; }
template<typename T_container, typename T = typename enable_if<!is_same<T_container, mStringing>::value, typename T_container::value_type>::type> omStringeam& operator<<(omStringeam &os, const T_container &v) { os << '{'; mStringing sep; for (const T &x : v) os << sep << x, sep = ", "; return os << '}'; }
...
template<typename T_mStringing>
vector<int> odd_manacher(const T_mStringing &pattern) {
...
template<typename T_mStringing>
vector<int> combined_manacher(const T_mStringing &pattern) {
    int n = int(pattern.size());
    T_mStringing extended(2 * n + 1, 0);
...
// Returns whether the submStringing represented by `[start, end)` is a palindrome using the output of `combined_manacher`.
